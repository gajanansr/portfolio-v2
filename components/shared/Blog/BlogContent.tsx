...
const components = {
    ...
    td: (props: React.HTMLAttributes<HTMLTableCellElement>) => (
        <td {...props} className="px-4 py-3 text-sm border-t border-neutral-200 dark:border-neutral-800" />
    ),
    strong: (props: React.HTMLAttributes<HTMLElement>) => (
        <strong {...props} className="font-bold text-neutral-900 dark:text-neutral-100" />
    ),
...
export default function BlogContent({ content }: BlogContentProps) {
    ...
    const fallbackContent = useMemo(() => {
        ...
        return content
            ...
            .map((block, i) => {
                if (block.startsWith("# ")) {
                    return <h1 key={i} className="text-3xl font-bold mt-8 mb-4">{block.slice(2)}</h1>;
                }
                if (block.startsWith("## ")) {
                    return <h2 key={i} className="text-2xl font-bold mt-8 mb-4">{block.slice(3)}</h2>;
                }
                if (block.startsWith("### ")) {
                    return <h3 key={i} className="text-xl font-bold mt-6 mb-3">{block.slice(4)}</h3>;
                }
