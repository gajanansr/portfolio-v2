...
- **Partitioning** → *Splitting one database into parts* (one pizza cut into 8 slices)
- **Sharding** → *Splitting data across multiple databases / servers* (8 different pizzas)
...
- Person 1 eats
- Then person 2 waits
- Then person 3 waits. …you get the pain.
...
├── Partition 1 → userId 1–100
├── Partition 2 → userId 101–200
├── Partition 3 → userId 201–300
```
...
2. Routing logic decides → **Shard 2 (Europe)**
3. Query goes **directly** to that shard
4. Other shards stay chilling 😎
...
- Traffic is crazy
- One DB can’t handle load
- You have global users
- You want fault isolation

**Real talk:**

> Most startups don’t need sharding on day one.
> 
> 
...
```
Shard (by region)
└── Each shard has partitioned tables
```
...
- **Partitioning** → Cutting one pizza into slices
- **Sharding** → Ordering more pizzas and spreading them across the table
- If your pizza is big → slice it
- If no slices left → order more pizzas 🍕🍕🍕.